...
void control_car(PID_Handler wheels_speed_pid[4])
{
    ...
    for (int i=0;i<4;++i) {
        input[i]=PID_output(&wheels_speed_pid[i],target_speed[i],get_wheel_velocity(i));
        if (input[i] > 32760) {input[i] = 32760;}
        if (input[i] < -32760) {input[i] = -32760;}
    }
